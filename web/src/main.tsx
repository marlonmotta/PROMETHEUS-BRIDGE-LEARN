  ...
  <StrictMode>
    <SentryErrorBoundary fallback={
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100vh", fontFamily: "sans-serif", color: "#ccc", background: "#0d0d0d" }}>
        <h1 style={{ fontSize: "1.5rem", marginBottom: "0.5rem" }}>{t("errorBoundary.title")}</h1>
        <p style={{ fontSize: "0.9rem", marginBottom: "1rem" }}>{t("errorBoundary.description")}</p>
        <button onClick={() => window.location.reload()} style={{ padding: "8px 20px", borderRadius: "4px", background: "#8b5cf6", color: "#fff", border: "none", cursor: "pointer" }}>{t("errorBoundary.reload")}</button>
      </div>
    }>
      <BrowserRouter>
        <ErrorBoundary>
