...
export default function ResultView({
  ...
  /** Mapeamento de extensões para filtros do dialog nativo */
  const FORMAT_FILTERS: Record<string, { name: string; extensions: string[] }> = {
    ...
    pdf: { name: "Documento para impressão (HTML)", extensions: ["html"] },
  };
...
<body>
  ...
  /**
   * Exporta o resultado para um arquivo no formato especificado.
   *
   * Usa o dialog nativo do sistema operacional para o professor escolher
   * onde salvar. O diretório padrão é Documentos.
   *
   * - TXT/MD: gravação direta do texto via backend Rust
   * - HTML: converte Markdown para HTML sanitizado e grava
   * - DOCX: gera documento Word nativo via `docx-rs` no backend
   * - PDF: gera PDF real via html2pdf.js a partir do template de prova
   */
  async function exportAs(format: string) {
    const text = result || fullPrompt;
    if (!text) return;

    // Abre o dialog nativo de salvar arquivo
    const filePath = await save({
      defaultPath: `adaptacao-pbl.${format}`,
      filters: [FORMAT_FILTERS[format] || { name: format, extensions: [format] }],
      title: "Salvar adaptação",
    });

    // Professor cancelou o dialog
    if (!filePath) return;

    try {
      if (format === "pdf") {
        // Gera HTML template estilo prova escolar
        const htmlTemplate = buildExamTemplate(text);
        await invoke("export_file", { content: htmlTemplate, format: "html", path: filePath });
        toast(
          "Documento salvo! Abra o arquivo no navegador e use Ctrl+P para gerar o PDF.",
          "info",
        );
      } else if (format === "html") {
        // HTML simples (sem estilo de prova)
        let parsedHtml: string;
        try {
        ...
        const sanitizedHtml = DOMPurify.sanitize(parsedHtml);
        const html = `<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Adaptação PBL</title></head><body><div style="font-family:sans-serif;max-width:800px;margin:40px auto;line-height:1.8">${sanitizedHtml}</div></body></html>`;
        await invoke("export_file", { content: html, format: "html", path: filePath });
      } else {
        // TXT, MD e DOCX - delega ao backend Rust
        await invoke("export_file", { content: text, format, path: filePath });
      }
  ...
  return (
    <section id="print-area">
      <div className="mb-8 no-print">
        ...
        <p className="text-sm text-txt-2">Conteúdo adaptado no universo da persona</p>
      </div>

      <div className="flex gap-2 mb-5 no-print">
        <span className="bg-accent/[0.12] border border-accent/20 text-accent text-[10px] px-2 py-0.5 rounded-full">
      ...
      {isManual && (
        <div>
          <Block
            ...
            onCopy={copy}
          />
          <Block title="Instrução Completa (cole na IA)" text={fullPrompt} onCopy={copy} />
          <p className="text-xs text-txt-3 py-2.5 flex items-center gap-1.5">
            <Icon name="info" size={14} className="text-txt-3 shrink-0" />
            Cole o <strong>System Prompt</strong> no campo de sistema da IA, depois cole a{" "}
            <strong>Instrução Completa</strong> como mensagem.
          </p>
        </div>
      )}
      ...
      {!isManual && (
        <div>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: renderSanitizedMarkdown(result) }}
          />
        </div>
      )}
...
function Block({
  ...
  return (
    <div className="bg-bg-2 border border-border rounded mb-5 overflow-hidden">
      ...
      <pre className="p-4 text-[13px] leading-relaxed text-txt whitespace-pre-wrap break-words max-h-80 overflow-y-auto">
        {text}
      </pre>
    </div>
  );
