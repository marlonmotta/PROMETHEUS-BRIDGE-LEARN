    ...
    <div className="fixed bottom-4 right-4 z-[60] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div
          key={t.id}
          role="alert"
          className={`pointer-events-auto flex items-center gap-2.5 px-4 py-3 rounded border shadow-[0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-sm text-[13px] font-medium animate-slide-in ${colorMap[t.type]}`}
        >
          <Icon name={iconMap[t.type] as keyof typeof iconMap} size={15} />
          <span>{t.message}</span>
          <button
